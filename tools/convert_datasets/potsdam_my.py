...
def clip_big_image(image_path, clip_save_dir, args, to_label=False):
    ...
    if to_label:
        ...
        image = out.reshape(h, w).astype(np.uint8)
...
def main():
    ...
    zipp_list = glob.glob(os.path.join(dataset_path, '*'))
    print('Find the data', zipp_list)

    for zipp in zipp_list:
        tmp_dir = zipp
        # with tempfile.TemporaryDirectory(dir=zipp) as tmp_dir:
