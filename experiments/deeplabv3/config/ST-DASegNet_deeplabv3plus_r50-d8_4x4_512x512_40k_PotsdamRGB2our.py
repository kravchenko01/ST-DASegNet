_base_ = [
    '../../../configs/_base_/datasets/our.py', '../../../configs/_base_/default_runtime.py'
]
...
model = dict(
    ...
    decode_head_s=dict(
        ...
        align_corners=False,
        loss_decode=dict(
            type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1.0, class_weight=[1.0, 1.0])), #class_weight=[1.0, 1.0, 1.0, 1.25, 1.5, 1.5])),
    decode_head_t=dict(
        type='DepthwiseSeparableASPPHead',
        in_channels=2304,
        in_index=3,
    ...
    discriminator_t=dict(
     ...
     cross_EMA = dict(
        ...
        pseudo_class_weight=[1.01, 1.01], #[1.01, 1.01, 1.51, 1.51, 2.01, 2.01],
        backbone_EMA=dict(
                type='ResNetV1c',
                depth=50,
                num_stages=4,
...
data = dict(samples_per_gpu=3, workers_per_gpu=3, 
            train=dict(img_dir='postdam/img_dir/train', ann_dir='postdam/ann_dir/train', split='postdam/train.txt'))
total_iters = 40000
